...
// Maximum allowed timeout for exec operations to prevent abuse
const MaxExecTimeout = 1 * time.Minute

// defaultExecInPod is the default implementation of ExecInPodFunc
func (c *Client) defaultExecInPod(ctx context.Context, namespace, name string, command []string, container string, timeout time.Duration) (*unstructured.Unstructured, error) {
	if name == "" {
...
func (c *Client) PostResource(ctx context.Context, gvr schema.GroupVersionResource, namespace, name, subresource string, body map[string]interface{}, parameters map[string]string) (*unstructured.Unstructured, error) {
	if name == "" {
